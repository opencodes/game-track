...
export const Leaderboard: React.FC = () => {
    ...
    const fetchLeaderboard = async () => {
      ...
      const res = await fetch(`${API_BASE_URL}/api/leaderboard`);
        if (res.ok) {
          ...
          const mapped = rows.map((p: any, i: number) => ({
            ...p,
            rank: i + 1,
            trend: 'stable',
  ...
  const remainingPlayers = players.length > 3 ? players.slice(3) : players;
              ...
              >
                ...
                <div className="col-span-5 sm:col-span-4 flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-white/5 border border-white/10 p-0.5 group-hover:border-gaming-accent transition-colors">
                    <img src={player.avatar} alt={player.displayName || player.username} className="w-full h-full rounded-md" referrerPolicy="no-referrer" />
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="font-bold truncate">{player.displayName || player.username}</span>
                    <AnimatePresence>
                      {player.lastChange && (
                        <motion.div
                ...
                <div className="col-span-2 text-center font-mono text-xs text-white/80">{Number(player.xp || 0).toLocaleString()}</div>
