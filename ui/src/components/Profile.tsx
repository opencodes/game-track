...
export const Profile: React.FC = () => {
  const { user, games, achievements, playSound, logout, refreshUser } = useGame();
  ...
  useEffect(() => {
    if (user) {
      ...
      setAge(user.age !== null && user.age !== undefined ? user.age.toString() : '');
  ...
  const activeAvatar = (isEditing ? avatar : user.avatar) || user.avatar;
          ...
          <div className="hud-panel p-6 flex flex-col items-center text-center">
            <div className="relative mb-4 group">
              <div className="w-32 h-32 rounded-3xl border-2 border-gaming-accent p-1 bg-black/40">
                <img src={activeAvatar} alt="Avatar" className="w-full h-full rounded-2xl" referrerPolicy="no-referrer" />
              </div>
        ...
        >
          <div className="hud-panel p-8 hud-border">
            ...
            <div className="space-y-6">
              <div className="grid sm:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <p className="text-lg font-bold">{user.age !== null && user.age !== undefined ? `${user.age} Years` : '—'}</p>
                  )}
                </div>
              </div>
              ...
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-widest text-white/40 mb-2">Avatar URL</label>
                {isEditing ? (
                      ...
                      {avatars.map((url) => (
                        <button
                          ...
                          className={`w-10 h-10 rounded-xl border ${
                            avatar === url ? 'border-gaming-accent' : 'border-white/10'
                          } overflow-hidden bg-black/40 hover:border-gaming-accent transition-colors`}
                        >
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl border border-white/10 overflow-hidden bg-black/40">
                      <img src={user.avatar} alt="Avatar" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                    </div>
                    <p className="text-sm font-semibold text-white/70 truncate">{user.avatar}</p>
                  </div>
                )}
              </div>
