...
const AppContent: React.FC = () => {
  ...
  const renderContent = () => {
    if (!isLoggedIn && activeTab !== 'home' && activeTab !== 'leaderboard' && activeTab !== 'register') {
      return <Login onLogin={() => setActiveTab('dashboard')} onRegister={() => setActiveTab('register')} />;
    }
    ...
    switch (activeTab) {
      ...
      case 'login':
        return <Login onLogin={() => setActiveTab('dashboard')} onRegister={() => setActiveTab('register')} />;
      case 'register':
        return <Register onRegistered={() => setActiveTab('dashboard')} onGoLogin={() => setActiveTab('login')} />;
      case 'dashboard':
        return <Dashboard />;
      case 'tracker':
        return <GameTracker />;
